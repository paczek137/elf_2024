...
struct Letters
{
    ...
    bool CheckXMAX2(NodeLetter &letter)
    {
        if ((letter.up == nullptr) or (letter.down == nullptr) or (letter.right == nullptr) or (letter.left == nullptr)) return false;
        // std::cout << "pass\n";
        if ( (((letter.up->left->letter == 'M') and (letter.down->right->letter == 'S')) or ((letter.up->left->letter == 'S') and (letter.down->right->letter == 'M'))) and
             (((letter.up->right->letter == 'M') and (letter.down->left->letter == 'S')) or ((letter.up->right->letter == 'S') and (letter.down->left->letter == 'M')))) return true;
        return false;
    }
    void CountXMAS()
    {
        ...
        for (size_t i=0; i<this->letters.size(); i++)
        {
            if (this->letters[i].letter != 'A') continue;
            // std::cout << "i: " << i << "\n";
            if (this->CheckXMAX2(this->letters[i])) count++;
...
int main()
{
    ...
    // string filename("example_input"); // 9
