...
struct Pages
{
    ...
    bool CheckRow(const PageNumbers &pages)
    {
        ...
        for (const auto &rule : this->rules)
        {
            ...
            {
                std::cout << "wrong page order in: " << pages[0] << ", " << pages[1] << "...\n";
                return false;
                // std::cout << "matching rule first: " << rule.before << ", after: " << rule.after << "\n";
                // PageNumbers pages2 = pages;
                // too naive approach
                // std::iter_swap(std::find(pages2.begin(), pages2.end(), rule.after), std::find(pages2.begin(), pages2.end(), rule.before));

                // return *(pages2.begin() + pages2.size() / 2);
            }
        }
        return true;
    }

    void RepairRow(PageNumbers &pages)
    {
        while (not this->CheckRow(pages))
        {
            for (const auto &rule : this->rules)
            {
                std::vector r = {rule.before, rule.after};
                ...
                if (std::find(pages.cbegin(), pages.cend(), rule.after) < std::find(pages.cbegin(), pages.cend(), rule.before))
                {
                    std::iter_swap(std::find(pages.begin(), pages.end(), rule.after), std::find(pages.begin(), pages.end(), rule.before));
                    break;
                }
            }
        }
    ...
    void Check()
    {
        ...
        for (const auto &row : this->pages)
        {
            ...
            PageNumbers row2 = row;
            this->RepairRow(row2);

            total = total +  *(row2.begin() + row2.size() / 2);
        }
...
int main()
{
    ...
    // string filename("example_input"); // 123
